...
export function useDocument<T>(path: string) {
	const { data, error, mutate } = useSWR<T | null, Error>(path, null, {
		refreshInterval: 0,
		fetcher: (path: string) => {
...
export function useCollection<T>(path: string) {
	const { data, error, mutate } = useSWR<T[]>(path, null, {
		refreshInterval: 0,
		fetcher: (path: string) => {
			return new Promise((resolve, reject) => {
				const collectionRef = collection(db, path)
						...
						(doc) => {
							if (doc.exists()) {
								const currentData = {
									id: doc.id,
									...doc.data(),
								} as SimplePerpWithDelta
								if (
									previousData &&
									currentData.markPrice !== previousData.markPrice
								) {
									const currentMarkPrice =
										parseFloat(currentData.markPrice) / Math.pow(10, 6)
									const previousMarkPrice =
										parseFloat(previousData.markPrice) / Math.pow(10, 6)
